...
console.log(`The tyrannosaurus weighed ${tyrannosaurus.weight}kgs.`);
...
graduates.forEach( function cb(elementor){
  universities.push(elementor.university)
})
universities.sort()
...
const contactInfo = []
graduates.forEach( function cb(elementor){
  contactInfo.push(`${elementor.first_name} ${elementor.email}` )
})
...
const unisWithUni = universities.filter(function cb(elementor, index){
  

  return elementor.search("Uni") >=0
...
const lowPopulationAnimals = zooAnimals.filter(el=>el.population <5)
...
const populationTotal = zooAnimals.reduce(function cb(acc, item){
